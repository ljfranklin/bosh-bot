...
NAN_SETTER(GitRemoteCallbacks::SetTransferProgress)
{
  ...
  if (callback) {
    ...
    wrapper->transfer_progress.SetCallback(callback, throttle);
  }
  ...
  const git_transfer_progress * stats,           void * payload        )
{
  ...
  return baton->ExecuteAsync((uv_async_cb) transfer_progress_async);
}
...
void GitRemoteCallbacks::transfer_progress_async(uv_async_t* req, int status)
{
  ...
  Nan::TryCatch tryCatch;
  Local<v8::Value> result = instance->transfer_progress.GetCallback()->Call(2, argv);
